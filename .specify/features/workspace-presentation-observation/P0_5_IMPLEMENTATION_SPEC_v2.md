...
**Quy tắc P0.5:** trước khi gọi `isOnCurrentScreen(windowID:)`, **nếu** placement vừa đi qua `exitFullScreen` thành công ở bước `prepare`, PHẢI re-resolve `CGWindowID` bằng pattern `resolveWindowID(for: pid, frame:)` đã có sẵn trong `AXAccessibilityService.swift` (Phase 1 mục #14), thay vì tái dùng `windows[0].window.id` capture từ đầu chuỗi resolve.
...
3. [NẾU step 2 đã qua exitFullScreen thành công]:
     resolvedCGWindowID = resolveWindowID(for: pid, frame: <frame post-exit>)
...
- **Setup**: 1 window đang fullscreen (fullscreen stub). Trước `exitFullScreen`, `ManagedWindow.id == 100`. Sau `exitFullScreen` thành công, mock `resolveWindowID(for pid: frame:)` trả `CGWindowID == 200` (ID mới). Mock `CurrentScreenVisibilityChecking`:
...
- **Expected**:
  ...
  - `summary.placedCount == 0`.
  - Số lần gọi `resolveWindowID` (mock bằng `resolveWindowIDCallCount`): **1** (chỉ re-resolve sau fullscreen-exit, không phải mỗi placement).
- **Variant T14-fail** (sub-test trong cùng file): mock `resolveWindowID` trả `nil` (không tìm được ID mới khớp pid+frame).
...
- **Thay đổi**:
  ...
  - Thứ tự trong `place(...)` (cập nhật Phase 4.5):
    ...
    2. **Nếu** `prepare` đã qua `exitFullScreen` thành công: re-resolve `CGWindowID` qua `accessibilityService.resolveWindowID(for: pid, frame:)` (Phase 1 #14). Nếu re-resolve trả `nil` → trả `.unverifiable(reason: .presentationUnverifiable)`, **không** gọi observation.
...
| **CGWindowID có thể đổi sau `exitFullScreen`** (Chromium/Electron huỷ + tạo lại window) | Re-resolve `CGWindowID` qua `resolveWindowID(for:frame:)` trước observation khi placement vừa qua fullscreen-exit (xem Phase 4.5, Step 5, test T14). Nếu re-resolve fail → `.unverifiable`, KHÔNG suy diễn `.movedButNotPresented`. |
