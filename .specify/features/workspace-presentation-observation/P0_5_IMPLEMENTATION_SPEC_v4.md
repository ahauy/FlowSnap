...
///
/// Two methods:
///   * `isOnCurrentScreen(windowID:)` — primary lookup by `CGWindowID` (the window number
///     captured at `matchingWindows` time). Returns an `OnScreenObservationResult`
///     enum carrying `presented` / `notPresented` / `unverifiable(reason:)`.
///
///   * `reResolveWindowID(pid:frame:)` — secondary lookup by `(pid, frame)`. Used
///     exclusively when a placement has just been through `exitFullScreen` and the
///     app may have destroyed + recreated the window (CGWindowID no longer valid).
///     See §4.5.
///
...
Đã rà soát đồng bộ toàn bộ chữ ký protocol — sẵn sàng implement.
