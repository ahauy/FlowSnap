...
| **US-DISP-017** | **Atomic Workspace Cross-Display Migration**                     | • Single-keystroke entire workspace cross-screen migration (`⌃⌥⇧⌘→`)<br>• Proportional multi-display scaling (`RelativeFrameScaler`)<br>• Stage Manager cohesion & 2-phase move ordering<br>• Cursor warping & adaptive divider reset     | [View Guide](workspace-cross-display-migration.md) |
...
| **US-SNAP-024** | **Launch FlowSnap at Login (`SMAppService`)**                    | • Modern macOS `SMAppService.mainApp` registration<br>• Two-way synchronization with macOS Login Items<br>• `.requiresApproval` status alert & deep link<br>• Zero helper app / single-app architecture                                   |          [View Guide](launch-at-login.md)          |
...
| **US-WEB-025**  | **FlowSnap Web Portal & Base Layout**                            | • Astro static generation & sub-1s load<br>• Zero-FOUC Dark/Light mode switching (`#09090b` canvas)<br>• Sticky liquid glass header & MIT open-source footer<br>• Apple HIG system typography stack                                       |        [View Guide](web-baseline-layout.md)        |
| **US-WEB-026**  | **3D Interactive macOS Desktop Simulator**                       | • Hardware-accelerated CSS 3D perspective tilt (clamped ±4.5°)<br>• Freeform draggable mock window with Swift 6 snippet<br>• Windows 11-style Top-Edge Snap Picker (4 templates)<br>• Liquid glass HUD Snap Preview & 0.0% idle CPU pause |        [View Guide](web-desktop-sandbox.md)        |
...
- [`images/always-on-top-window-pinning/`](images/always-on-top-window-pinning/)
