...
public struct CollinearEdgeDetector: CollinearEdgeDetecting, Sendable {
    ...
    public init(defaultMinWidth: CGFloat = 200.0, defaultMinHeight: CGFloat = 150.0) {
    ...
    public func detectDividers(
        ...
        for i in 0..<vPairs.count {
            ...
            var minCoord: CGFloat = containerFrame.minX
            for w in leadingWindows {
                let minW = w.minSize?.width ?? defaultMinWidth
                minCoord = max(minCoord, w.frame.minX + minW + gap / 2.0)
            }

            var maxCoord: CGFloat = containerFrame.maxX
        ...
        for i in 0..<hPairs.count {
            ...
            var minCoord: CGFloat = containerFrame.minY
            for w in leadingWindows {
                let minH = w.minSize?.height ?? defaultMinHeight
                minCoord = max(minCoord, w.frame.minY + minH + gap / 2.0)
            }

            var maxCoord: CGFloat = containerFrame.maxY
            for w in trailingWindows {
                let minH = w.minSize?.height ?? defaultMinHeight
    ...
    public func computeResizedFrames(
        ...
        let clampedCoordinate = max(divider.minCoordinate, min(divider.maxCoordinate, targetCoordinate))
        ...
        case .vertical:
            let newLeadingBoundary = clampedCoordinate - (gap / 2.0)
            let newTrailingBoundary = clampedCoordinate + (gap / 2.0)
            ...
            for id in divider.leadingWindowIDs {
                ...
                let newWidth = max(minW, newLeadingBoundary - window.frame.minX)
                result[id] = CGRect(
                    x: window.frame.minX,
                    y: window.frame.minY,
                    width: newWidth,
            ...
            for id in divider.trailingWindowIDs {
                guard let window = windowDict[id] else { continue }
                let minW = window.minSize?.width ?? defaultMinWidth
                let newWidth = max(minW, window.frame.maxX - newTrailingBoundary)
                result[id] = CGRect(
                    x: newTrailingBoundary,
                    y: window.frame.minY,
                    width: newWidth,
                    height: window.frame.height
                )
        ...
        case .horizontal:
            ...
            for id in divider.leadingWindowIDs {
                ...
                let newHeight = max(minH, newLeadingBoundary - window.frame.minY)
                result[id] = CGRect(
                    x: window.frame.minX,
                    y: window.frame.minY,
            ...
            for id in divider.trailingWindowIDs {
                guard let window = windowDict[id] else { continue }
                let minH = window.minSize?.height ?? defaultMinHeight
                let newHeight = max(minH, window.frame.maxY - newTrailingBoundary)
                result[id] = CGRect(
                    x: window.frame.minX,
                    y: newTrailingBoundary,
                    width: window.frame.width,
                    height: newHeight
                )
            }
