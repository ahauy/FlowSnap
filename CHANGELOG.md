...
## [1.4] — 2026-09-03 — IMPLEMENTATION COMPLETE

### Added — US-DISP-015: Cross-Display Window Throw & Target-Aware Snap

- **Domain**
  - `WindowCommand` extended: added `.moveToNextDisplay` and `.moveToPreviousDisplay`.
  - `ShortcutAction` updated: mapped `.nextDisplay` (default: `⌃⌥⇧→`) and `.previousDisplay` (default: `⌃⌥⇧←`).
- **Core**
  - `DisplayNavigator`: pure spatial topology engine ordering displays left-to-right (`minX`, tie-break `minY`) with cyclic modulo wrap-around.
  - `RelativeFrameScaler`: proportional geometric transformer mapping window bounds across displays with `FrameClampingHelper` integration.
  - `CommandDispatcher`: added `executeCrossDisplayThrow(isNext:)` pipeline combining semantic snap re-calculation, relative proportional scaling, AX movement, and focus re-assertion.
- **Infrastructure**
  - `CursorManager` & `CursorWarping`: instant mouse pointer relocation to target window center point via `CGWarpMouseCursorPosition`.
- **Verification**
  - 372 unit & integration tests passing across 58 suites.
  - SwiftLint clean (0 errors, 0 warnings).
  - 100% public macOS APIs (zero private CGS symbols).
